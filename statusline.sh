...
if [ "$COMPACT_MODE" = "true" ]; then
    STATUS_LINE="[$MODEL_DISPLAY] 📁 ${LOCATION_DISPLAY}$GIT_DISPLAY | ${CONTEXT_DISPLAY} | 🕐 ${SESSION_TIME} | 🪙  ${TOKEN_DISPLAY}${VELOCITY_DISPLAY}"
else
    STATUS_LINE="[$MODEL_DISPLAY] 📁  ${LOCATION_DISPLAY}$GIT_DISPLAY | ${CONTEXT_DISPLAY} | 🕐  Started $SESSION_TIME ago | 🪙  ${TOKEN_DISPLAY}${VELOCITY_DISPLAY}"
fi
